      ...
      <div className="flex flex-col h-full px-10 py-8 gap-4 overflow-hidden">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="text-white/80 text-[10px] border border-white/10 rounded-full px-2 py-0.5">LogsDB · flexible timestamps · 4 days historical</span>
          </div>
          <h2 className="text-4xl font-bold text-white leading-tight">
            Slow Query Intelligence <span className="text-white/70">From the Error Log Up</span>
          </h2>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }}
          ...
          {metrics.map((m, i) => (
            <motion.div key={m.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
              ...
              className="flex flex-col gap-1 p-4 rounded-xl border border-white/15 bg-white/3">
              <m.icon size={16} style={{ color: m.color }} />
              <div className="text-2xl font-bold text-white mt-1">{m.value}</div>
              <div className="text-white/85 text-xs">{m.label}</div>
              <div className="text-white/85 text-[10px]">{m.sub}</div>
            </motion.div>
          ))}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.45 }}
          ...
          <h3 className="text-white/85 text-xs font-semibold uppercase tracking-widest">Dashboard panels</h3>
          {capabilities.map((c, i) => (
            <motion.div key={c.text} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }}
              ...
              <span className="text-white/88 text-sm leading-snug">{c.text}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}
          ...
          <span className="text-white/80">Streams:</span>
          <code className="text-white/70">logs-mysql.slowlog.otel.otel-default</code>
          <span className="text-white/20">·</span>
          <code className="text-white/70">logs-mysql.error.otel.otel-default</code>
        </motion.div>
      </div>
    </SlideLayout>
  );
