...
2. Open the **MySQL**, **PostgreSQL**, **SQL Server**, **MongoDB**, **IBM Db2**, or **Oracle** performance dashboard: the bottom **AI recommendations** block has a markdown ribbon, an ES|QL **data table** for the latest markdown (so text is readable), and **Stored recommendation runs**; all filter on **`database_platform`**. Deployed dashboards default to **Last 1 minute** (dense OTLP); widen the time picker (e.g. **Last 15 minutes**) if the table is empty while **Stored runs** is non-zero. Older rows without **`database_platform`** still match the **Db2** dashboard only.
