...
2. Open the **MySQL**, **PostgreSQL**, **SQL Server**, **MongoDB**, **IBM Db2**, or **Oracle** performance dashboard: the bottom **AI recommendations** block is a **Markdown** panel when the importer found a row in **`db-monitoring-recommendations`** (text is fetched from Elasticsearch at deploy time and rendered as Markdown; Kibana cannot bind Markdown directly to live ES|QL). If the index was empty at deploy time, you see a short Markdown note plus a **metric** with live plain text. After the workflow runs, **re-run** `scripts/import_dashboards.py` (or your lab bootstrap) to refresh the Markdown snapshot. Deployed dashboards default to **Last 1 minute** (dense OTLP); widen the time picker (e.g. **Last 15 minutes**) if charts look empty. Older rows without **`database_platform`** still match the **Db2** dashboard only.
