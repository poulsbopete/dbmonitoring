...
2. Open the **MySQL**, **PostgreSQL**, **SQL Server**, **MongoDB**, **IBM Db2**, or **Oracle** performance dashboard: the bottom **AI recommendations** panel is a single ES|QL **metric** (latest markdown text only; Serverless rejects inline datatables on this API), filtered by **`database_platform`**. Deployed dashboards default to **Last 1 minute** (dense OTLP); widen the time picker (e.g. **Last 15 minutes**) if the cell is empty after a workflow run. Older rows without **`database_platform`** still match the **Db2** dashboard only.
